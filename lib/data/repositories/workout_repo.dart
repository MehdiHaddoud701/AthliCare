...
class WorkoutRepository {
  ...
  Future<DayScheduleModel?> getDayWithWorkouts(int dayId) async {
    try {
      print('🔍 Repository.getDayWithWorkouts called with dayId=$dayId');
      final db = await DBHelper.getDatabase();
      ...
      print('   📊 Query result: ${dayMaps.length} day records found');
      if (dayMaps.isEmpty) {
        print('   ❌ No day found with dayId=$dayId');
        return null;
      }
      ...
      print('   📦 Found ${workouts.length} workouts for dayId=$dayId');
      ...
      print('   ✅ Returning day with ${workoutsWithTasks.length} workouts');
  ...
  /// Creates days and assigns available workouts to them
  Future<WeeklyPlanModel?> generateWeeklyPlanForUser(
    ...
    try {
      ...
      // Get all available workouts to assign to days
      final availableWorkouts = await getAllWorkouts();
      print(
        '📦 Found ${availableWorkouts.length} available workouts for assignment',
      );

      // Create day schedules and assign workouts
      for (int i = 1; i <= activeDaysPerWeek; i++) {
        final dayId = await insertDaySchedule(
          DayScheduleModel(weeklyPlanId: weeklyPlanId, dayIndex: i),
        );

        // Assign workouts to this day (cycle through available workouts)
        if (availableWorkouts.isNotEmpty && dayId != -1) {
          // Assign 1-2 workouts per day based on availability
          final workoutIndex = (i - 1) % availableWorkouts.length;
          final workout1 = availableWorkouts[workoutIndex];

          if (workout1.workoutId != null) {
            await assignWorkoutToDay(dayId, workout1.workoutId!);
            print('   ✅ Day $i: Assigned workout "${workout1.title}"');
          }

          // If there are more than 2 workouts, assign a second one
          if (availableWorkouts.length > 1 && i % 2 == 0) {
            final workout2Index = (workoutIndex + 1) % availableWorkouts.length;
            final workout2 = availableWorkouts[workout2Index];
            if (workout2.workoutId != null) {
              await assignWorkoutToDay(dayId, workout2.workoutId!);
              print('   ✅ Day $i: Also assigned workout "${workout2.title}"');
            }
          }
        }
      }
